...
const artifactDir = path.join(root, "dist", "humanresource");
...
if (packagedEnvironment.PORT !== "3085") {
  throw new Error("The packaged frontend port is incorrect.");
}

const server = spawn(process.execPath, ["server.js"], {
  cwd: artifactDir,
  env: { ...process.env, HOSTNAME: "127.0.0.1", PORT: "3085" },
...
try {
  ...
  for (let attempt = 0; attempt < 60; attempt += 1) {
    ...
    try {
      pageResponse = await fetch("http://127.0.0.1:3085/hr-management/login");
      break;
    } catch {
      await delay(500);
    }
  ...
  if (!pageResponse?.ok) {
    throw new Error(`Packaged server did not return HTTP 200 for /hr-management/login.\n${output}`);
  }
  ...
  const runtimeResponse = await fetch("http://127.0.0.1:3085/runtime-config.js");
  ...
  console.log("humanresource production artifact smoke test passed:");
  console.log("- node server.js listened on port 3085");
  console.log("- GET /hr-management/login returned HTTP 200");
